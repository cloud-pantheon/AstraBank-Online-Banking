...
/* ------------------------
   LOAD USER INFO
-------------------------*/
$stmt = $conn->prepare("SELECT user_name, email FROM user WHERE cid = ?");
...
<head>
  <meta charset="utf-8">
  <title>Info Update — New Email</title>
  ...
  <style>
    ...
    .holder-avatar{ width:56px; height:56px; border-radius:999px; border:1px solid var(--border); 
      display:flex; align-items:center; justify-content:center; font-size:26px; color:var(--muted); background:#fbfbfc; }
    ...
    .otp-opt{ flex:1 1 120px; padding:10px 14px; border-radius:12px; border:1px solid var(--border); 
      background:#fff; display:flex; align-items:center; gap:8px; font-size:14px; cursor:pointer; justify-content:center; }
        ...
        <form method="post" action="info-update-process.php">
          ...
          <div class="row" style="margin-top:16px;">
            ...
            <div class="input-wrap">
              <input class="input" name="new_email" id="newEmail" type="email" placeholder="Enter new email" required>
            </div>
          </div>
